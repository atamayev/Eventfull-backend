...
export default async function deleteDBCalendarEvent (
	...
	try {
		...
		if (_.isNull(user)) throw new Error("Failed to delete calendar data")

		const event = user.calendarData.find(calendarEvent => calendarEvent.id === calendarId)

		if (_.isNil(event) || event.isActive === false) throw new Error("Failed to delete calendar data")
		...
		if (deleteType === "soft") event.isActive = false
			...
			if (eventIndex === -1) throw new Error("Failed to delete calendar data")
