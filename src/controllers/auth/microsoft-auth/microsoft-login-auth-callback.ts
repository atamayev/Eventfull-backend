...
export default async function microsoftLoginAuthCallback (req: Request, res: Response): Promise<Response> {
	try {
		...
		if (_.isUndefined(userClaims)) return res.status(500).json({ error: "Internal Server Error: Error authenticating Microsoft Token"})
		...
		if (_.isNull(userId)) return res.status(500).json({ error: "Internal Server Error: Error saving Microsoft Login Tokens" })

		const payload = createJWTPayload(userId)

		const token = signJWT(payload)
		if (_.isUndefined(token)) return res.status(500).json({ error: "Internal Server Error: Error with Signing JWT" })
